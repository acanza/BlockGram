...
class Instagram {
    ...
    */
    ...
    protected function _setOauthToken() {
        ...
        $this->_httpClient->setPostParam('redirect_uri', $this->_config['redirect_uri'].'?return_uri='.$this->_config['return_uri']);
        $this->_httpClient->setPostParam('scope', $this->_config['scope']);
        
        if( !empty( $this->_config[ 'code' ] ) ){
            
            $this->_httpClient->setPostParam('code', $this->_config[ 'code' ]);
        }else{
     ...
     * @return string
     */
    public function getAccessToken( $errorMessage ) {
        if ($this->_accessToken == null) {
            ...
            if ($this->_oauthToken == null) {
   			...
   			if (isset(json_decode($this->_oauthToken)->access_token)) {
   				
                            $this->_accessToken = json_decode($this->_oauthToken)->access_token;
   			}elseif(isset(json_decode($this->_oauthToken)->error_message)){
   				
   			    $errorMessage = json_decode($this->_oauthToken)->error_message;
   			}   
        }
    ...
    public function getCurrentUser() {
        if ($this->_currentUser == null) {
            ...
            $this->_currentUser = json_decode($this->_oauthToken , true);
            
            if (isset($this->_currentUser['user'])) {
    ...
    protected function getAccessCode() {
    	if(isset($_GET[self::RESPONSE_CODE_PARAM])){
    ...
    public function getAuthorizationUrl() {
        ...
        return sprintf($this->_endpointUrls['authorize'],
            ...
            $this->_config['redirect_uri'].'?return_uri='.$this->_config['return_uri'],
            $this->_config['scope'],
